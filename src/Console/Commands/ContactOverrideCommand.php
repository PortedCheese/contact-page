...
class ContactOverrideCommand extends Command
{
    ...
    {
        if (file_exists($controller = app_path("Http/Controllers/ContactPage/{$place}/ContactController.php"))) {
            if (! $this->confirm("The [{$place}/ContactController.php] controller already exists. Do you want to replace it?")) {
                return;
            }
        }

        if (! is_dir($directory = app_path("Http/Controllers/ContactPage/{$place}"))) {
            mkdir($directory, 0755, true);
        }

        file_put_contents(
            app_path("Http/Controllers/ContactPage/{$place}/ContactController.php"),
            $this->compileControllerStub($place)
        );

        $this->info("[{$place}/ContactController.php] created");
    }
    ...
    protected function compileControllerStub($place)
    {
        return str_replace(
            ...
            file_get_contents(__DIR__ . "/stubs/make/controllers/{$place}ContactController.stub")
        );
    }
}
